...
def str2():
    ...
    if st.button('Predict'):
        ...
        # Iterate over the ranges and check where thalach falls
        for min_val, max_val, label in ranges:
                if min_val <= thalach <= max_val:
                        ...
                        if label == 'Low':
                                ...
                                bg_color = 'red'
                        break
        st.markdown(f"<p style='background-color:{bg_color}; color:white; padding:10px;'>Prediction: {prediction_result}", unsafe_allow_html=True)
